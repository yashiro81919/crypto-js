...
export class Optimism implements Blockchain {
    ...
    private async getAddr(address: string): Promise<any> {
        const resp = await this.helper.api.get(`https://sandbox-api.3xpl.com/optimism/address/${address}?data=balances,events&from=all&limit=1000&library=currencies`);
        const balances = resp.data['data']['balances'];
        const events = resp.data['data']['events']['optimism-main'];
