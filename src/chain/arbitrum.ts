...
export class Arbitrum implements Blockchain {
    ...
    private async getAddr(address: string): Promise<any> {
        const resp = await this.helper.api.get(`https://sandbox-api.3xpl.com/arbitrum-one/address/${address}?data=balances&from=all&library=currencies,rates(usd)`);
        ...
        for (const token in erc20Obj) {
            ...
            if (rates[token]['usd']) {
                tokens.push({ name: tokenMeta[token]['symbol'], address: token.replace('arbitrum-one-erc-20/', '').toLowerCase(),
