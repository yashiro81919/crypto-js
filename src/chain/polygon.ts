...
export class Polygon implements Blockchain {
    ...
    private async getAddr(address: string): Promise<any> {
        const resp = await this.helper.api.get(`https://sandbox-api.3xpl.com/polygon/address/${address}?data=balances&from=all&library=currencies,rates(usd)`);
    ...
    private async getNonce(address: string): Promise<number> {
        const resp = await this.helper.api.get(`https://3xpl.com/polygon/address/${address}`);
